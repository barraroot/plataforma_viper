<template>
    <div class="block">
        <div v-if="(paymentType == null || paymentType === '') && wallet && setting">
            ...
            <div class="">
                <ul>
                    <li v-if="setting.suitpay_is_enable || setting.primepag_is_enable" @click="setPaymentMethod('pix')"
                    ...
                    <li v-if="setting.stripe_is_enable" @click="setPaymentMethod('stripe')"
        ...
        <div v-if="paymentType === 'pix' && setting && (setting.suitpay_is_enable || setting.primepag_is_enable)">
            <div v-if="showPixQRCode && wallet" class="flex flex-col ">
                <div class="w-full p-4 bg-white dark:bg-gray-700 rounded mb-3">
                    <div class="flex justify-between">
                        <h2 class="text-lg">Falta pouco! Escaneie o código QR pelo seu app de pagamentos ou Internet
                            Banking</h2>
                        <div class="text-4xl">
                ...
                <div class="w-full p-4">
                    <div>
                        <p class="font-bold">Valor do Pix a pagar:
                            {{ state.currencyFormat(parseFloat(deposit.amount), wallet.currency) }}</p>
                    </div>
                    ...
                    <form action="" @submit.prevent="submitQRCode">
                        ...
                        <div class="mt-3">
                            <p class="mb-2 text-gray-500">
                                ...
                                {{ state.currencyFormat(parseFloat(setting.max_deposit), wallet.currency) }}</p>
                            <div
                        ...
                        <div v-if="setting.rollover > 0" class="mt-3 text-gray-500">
                            <p>
                                <input type="checkbox"
                                       class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                       v-model="accept_bonus">
                                {{ $t('Accept an extra bonus') }} <strong
                                class="text-white font-bold">{{ setting.initial_bonus }}%</strong>
                                {{ $t('on a minimum deposit of') }} <strong class="text-white font-bold">{{
                                    state.currencyFormat(parseFloat(setting.min_deposit), wallet.currency)
                                }}</strong></p>
                        </div>
...
export default {
    ...
    methods: {
        ...
        submitQRCode: function (event) {
            ...
            _this.deposit.gateway = this.setting.suitpay_is_enable ? 'suitpay' : 'primepag';
            _this.deposit.accept_bonus = this.accept_bonus;
