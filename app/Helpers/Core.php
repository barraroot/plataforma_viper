...
class Core
{
     ...
     * @return void
     */
    public static function updateBalanceBetWithRollover($userId, $bet): string
    {
        ...
        if (!empty($wallet)) {
            ...
            if ($wallet->balance_bonus >= $bet && $wallet->balance_bonus > 0) {
                ...
                if (($wallet->balance_deposit_rollover - $bet) < 0) {
            ...
            if ($wallet->total_balance >= $bet) {
                if ($wallet->total_balance == $bet) {
                    ...
                    $percentBonus = ($wallet->balance_bonus / $bet) * 100;
                    $percentNormal = 100 - $percentBonus;
                    $wallet->update([
                        "balance" => 0,
                        "balance_bonus" => 0
                    ]);
                    return "all_balance_b_{$percentBonus}_s_{$percentNormal}";
                }
                ...
                if ($wallet->total_balance > $bet) {
                    ...
                    $valorBet = $wallet->balance + $wallet->balance_bonus;
                    if ($valorBet >= $bet) {
                        $valueBalance = ($bet - $valorBet) * -1;
                        $percentBonus = ($wallet->balance_bonus / $bet) * 100;
                        $percentNormal = 100 - $percentBonus;

                        $wallet->update(["balance_bonus" => 0, "balance" => $valueBalance]);
                        return "all_balance_b_{$percentBonus}_s_{$percentNormal}";
                    }
                }

            }
            throw new \Exception("Usuário sem saldo", 100);
        }
    }
    ...
    {
        Log::debug("Chegou aqui : "  . json_encode([$userId, $changeBonus, $WinAmount]));
        $wallet = Wallet::where('user_id', $userId)->first();
        if (!empty($wallet)) {
            ...
            if (Str::contains($changeBonus, "all_balance")) {
                $values = explode("_", $changeBonus);
                $saldo = ($values[5] / 100) * $WinAmount;
                $bonus = ($values[3] / 100) * $WinAmount;
                $wallet->increment("balance", $saldo);
     ...
     * @return mixed
     */
    public static function generateGameHistory($userId, $type, $win, $bet, $changeBonus, $tx, $skipUpdateTransaction = false)
    {
        ...
        if (floatval($win) > 0) {
            self::payWithRollover($userId, $changeBonus, $win);
        }

        if ($skipUpdateTransaction && $type == 'win') {
            $transaction = Order::where('transaction_id', $tx)->where('type', 'win')->first();
        ...
        if ($skipUpdateTransaction && $type == 'loss') {
            $transaction = Order::where('transaction_id', $tx)->where('type', 'loss')->first();
            if (!empty($transaction)) {
                if ($transaction->update(['status' => 1])) {
                    return true;
                }
    ...
    private static function PayLoss($user, $affiliate, $bet, $changeBonus)
    {
        ...
        if (!empty($affiliate)) {
            ...
            if (
                !empty($affHistoryRevshare)
                && (
                    in_array($changeBonus, ['balance', 'balance_withdrawal'])
                    || Str::contains($changeBonus, "all_balance")
                )
            ) {
                if (Str::contains($changeBonus, "all_balance")) {
                    $values = explode("_", $changeBonus);
                    $bet = ($values[5] / 100) * $bet;
                }
     ...
     * @return string
     */
    public static function amountPrepare($float_dollar_amount, $cent = false)
    {
        ...
        if ($cent) {
            return $float_dollar_amount * 100;
        } else {
