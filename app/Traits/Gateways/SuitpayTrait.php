...
trait SuitpayTrait
{
    ...
    public static function requestQrcode($request)
    {
        ...
        $response = Http::withHeaders([
            ...
            "callbackUrl" => url('/suitpay/webhook'),
            "client" => [
...
//            ]
        ...
        if ($response->successful()) {
            ...
            self::generateDeposit($responseData['idTransaction'], \Helper::amountPrepare($request->amount), $request->accept_bonus); /// gerando deposito
    ...
    public static function finalizePayment($idTransaction): bool
    {
        ...
        if (!empty($transaction)) {
            ...
            if (!empty($wallet)) {
                ...
                $rollover = $transaction->price * intval($setting->rollover_deposit);

                if ($transaction->accepted_bonus) {
                    $bonus = Helper::porcentagem_xn($setting->initial_bonus, $transaction->price);
                    $wallet->increment('balance_bonus', $bonus);
                    $rollover = $bonus * $setting->rollover;
                }

                $rollover += $wallet->balance_deposit_rollover;
                $wallet->update(['balance_deposit_rollover' => $rollover]);
     ...
     * @return void
     */
    private static function generateDeposit($idTransaction, $amount, $acceptedBonus)
    {
     ...
     * @return void
     */
    private static function generateTransaction($idTransaction, $amount, bool $acceptedBonus)
    {
        $setting = \Helper::getSetting();
