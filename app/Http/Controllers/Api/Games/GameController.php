...
use App\Traits\Providers\GamesApi;
...
{
    use KaGamingTrait, FiversTrait, VibraTrait, SalsaGamesTrait, WorldSlotTrait, Games2ApiTrait, GamesApi;
    ...
    public function sourceAction(Request $request, string $action)
    {
        $controller = \Helper::createController($request->game);
        return match ($action) {
            "win" => $controller->win($request),
            "info" => $controller->getInfo($request),
            "loss" => $controller->loss($request),
            default => response()->json(["error" => true, "message" => "Action inválida!"])
        };
    }
    ...
    public function show(Request $request, string $id)
    {
        ...
        if (!empty($game)) {
            if (auth('api')->check()) {
                ...
                $bet = $request->aposta ?? 0;
                if (
                    $game->technology === 'aposta'
                    && $bet <= 0
                ) {
                    return response()->json(['error' => 'Você precisa informar um valor de aposta para jogar', 'status' => false], 400);
                }
                if ($wallet->total_balance > 0) {
                    ...
                    switch ($game->distribution) {
                        case 'source':
                            $time = time();
                            if ($game->technology === 'aposta') {
                                try {
                                    $changeBonus = Helper::updateBalanceBetWithRollover(auth('api')->id(), $bet);
                                } catch (\Throwable $exception) {
                                    return response()->json(['error' => 'Você precisa ter saldo para jogar', 'status' => false, 'action' => 'deposit']);
                                }
                            ...
                            return response()->json([
                                'game' => $game,
                                'gameUrl' => url('/originals/' . $game->game_code . '/index.html?token=' . $token . '&session=' . $time),
                                'token' => $token
                            ]);
                        case 'salsa':
                            return response()->json([
                        ...
                        case 'games_api':
                            ...
                            if ($game->provider->code === "pg") {
                                return response()->json([
                                    'game' => $game,
                                    'gameBody' => self::play($request, $game),
                                    'token' => $token
                                ]);
                            }
                            ...
                            return response()->json([
                                'game' => $game,
                                'gameUrl' => self::play($request, $game),
                                'token' => $token
                            ]);
                        case 'kagaming':
                            return response()->json([
