        ...
        if (auth('api')->check()) {
            ...
            if (!empty($setting->withdrawal_limit)) {
                ...
                switch ($setting->withdrawal_period) {
                    case 'daily':
                        $registrosDiarios = $query->whereDate('created_at', now()->toDateString())->count();
                        ...
                        break;
                    case 'weekly':
                        $registrosDiarios = $query->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->count();
                        if ($registrosDiarios >= $setting->withdrawal_limit) {
                            return response()->json(['error' => trans('You have already reached the weekly withdrawal limit')], 400);
                        }
                        break;
                    case 'monthly':
                        $registrosDiarios = $query->whereYear('created_at', now()->year)->whereMonth('data', now()->month)->count();
                        if ($registrosDiarios >= $setting->withdrawal_limit) {
                            return response()->json(['error' => trans('You have already reached the monthly withdrawal limit')], 400);
                        }
                        break;
                    case 'yearly':
                        $registrosDiarios = $query->whereYear('created_at', now()->year)->count();
                        if ($registrosDiarios >= $setting->withdrawal_limit) {
            ...
            if ($request->accept_terms == true) {
                ...
                if ($user->wallet->balance_deposit_rollover > 0) {
                    return response()->json(['error' => "Você precisa cumprir o rollover de: {$user->wallet->balance_deposit_rollover}"], 400);
                }

                if (floatval($request->amount) > floatval($user->wallet->balance)) {
